(function () {
  ...
  document.querySelector("#download").addEventListener("click", function () {
    ...
    C.downloadBlob(
      ...
      C.baseName(sourceFile.name) +
        "-compressed." +
        C.extensionFor(outputType || outputBlob.type)
    );
  });
  ...
  async function selectFile(file) {
    ...
    [targetInput, formatInput, resizeInput, input].forEach(function (node) {
      node.disabled = false;
    });
  ...
  async function findBest(canvas, type, targetBytes, token) {
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      const blob = await C.canvasToBlob(
        canvas,
        type,
        quality
      );
  ...
  reused = true;
      ...
      if (reused) {
        ...
        C.setStatus(
          status,
          "已经尽量压缩，但仍高于目标大小。可以提高目标 KB，或先使用尺寸工具缩小图片。",
          "warn"
        );
      }
    } catch (error) {
