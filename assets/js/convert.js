(function () {
  ...
  quality.addEventListener("input", function () { qualityValue.textContent = quality.value + "%"; invalidate(); });
  [format, background].forEach(function (node) { node.addEventListener("input", function () { syncControls(); invalidate(); }); });
  runButton.addEventListener("click", convert);
  document.querySelector("#reset").addEventListener("click", reset);
  document.querySelector("#download").addEventListener("click", function () {
    if (outputBlob) C.downloadBlob(outputBlob, C.baseName(sourceFile.name) + "-converted." + C.extensionFor(outputBlob.type));
  });
  ...
  async function selectFile(file) {
    ...
    [format, quality, background, input].forEach(function (node) { node.disabled = false; });
  ...
  async function convert() {
    ...
    if (format.value === loaded.type) {
      C.setStatus(status, "源格式与目标格式相同。请选择另一种格式；需要减小体积可使用图片压缩。", "warn");
      return;
    }
    const token = gate.start();
    const controls = [format, quality, background, input, document.querySelector("#reset")];
    clearResult();
    C.setBusy(runButton, true, "转换处理中…");
    controls.forEach(function (node) { node.disabled = true; });
    C.setStatus(status, "正在转换格式。", "ok");
    try {
      const alpha = format.value !== "image/jpeg";
      const made = C.makeCanvas(loaded.width, loaded.height, alpha);
      if (!alpha) {
        made.ctx.fillStyle = background.value;
      ...
      document.querySelector("#original-format").textContent = loaded.type;
      ...
      C.setStatus(status, "格式转换完成，请检查结果后下载。", "ok");
    } catch (error) {
      if (gate.active(token)) C.setStatus(status, "转换失败：" + error.message, "danger");
    } finally {
      if (gate.active(token)) { controls.forEach(function (node) { node.disabled = false; }); C.setBusy(runButton, false, "开始转换"); runButton.disabled = !loaded; }
    }
  }
